// @ts-nocheck — Web Bluetooth + WebSocket sim mode. Untyped on purpose,
// see ConnectionManager.ts for the same rationale.
//
// BLEService.ts — Web Bluetooth connection to Inversa controller.
...
// We don't model the Web Bluetooth surface here — these fields are
// loosely-typed `any` because the API is browser-only and the legacy
// code worked off ad-hoc shapes. Strict typing isn't worth the cost
// during the port; we'd only re-type if we shared this with native.
class BLEServiceClass {
  ...
  _rxBuffer:     string = '';
  _requestMap:   Map<string, (response: any) => void> = new Map();
  _requestId:    number = 0;
