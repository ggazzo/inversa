// parseRecipe.ts — minimal mirror of firmware/src/plugins/RecipePlugin.h's
...
function parseLine(line: string): RecipeStep | null {
    const raw = line.trim();
    ...
    if (upper.startsWith('WAIT_TIMER')) {
        ...
        return { type: 'WAIT_TIMER', label: `Aguardar ${m} min`, value: m, kind: 'timer', raw };
    }
    if (upper.startsWith('TIMER')) {
        const m = parseFloat(raw.slice(5));
        return { type: 'TIMER', label: `Timer ${m} min`, value: m, kind: 'timer', raw };
    }
    ...
    if (upper.startsWith('BOIL')) {
        ...
        return { type: 'BOIL', label: `Ferver ${m} min`, value: m, kind: 'boil', raw };
    }
    if (upper.startsWith('ADD_HOP')) {
        ...
        return { type: 'ADD_HOP', label: `Lúpulo: ${name}`, value: min, message: name, kind: 'hop', raw };
    }
    ...
    if (upper.startsWith('RAMP')) {
        ...
        return { type: 'RAMP', label: `Rampa ${r}°C/min`, value: r, kind: 'ramp', raw };
    }
    ...
    if (upper.startsWith('WAIT_CONFIRM')) {
        ...
        return { type: 'WAIT_CONFIRM', label: msg || 'Confirmação manual', message: msg, kind: 'confirm', raw };
    }
...
export function parseRecipe(content: string): RecipeStep[] {
    if (!content) return [];
