...
//
// Dispatch is table-driven: handle() resolves the command `type` against
// a static {string -> member fn} table instead of a long if/else-if
// strcmp chain. Each command body lives in its own out-of-line method so
// the compiler no longer inlines all 60+ arms (incl. their ArduinoJson
// response builders) into one giant function — this shrinks .flash.text.
...
private:
    // ── Temperature ─────────────────────────────────────
    void cmdSetTemp(const String& rid, JsonDocument& doc) {
    ...
    // ── Heater ──────────────────────────────────────────
    void cmdHeaterOn(const String& rid, JsonDocument& doc) {
        EventBus::instance().publish(EventType::HeaterStateChanged, true);
        gState.mode = OperatingMode::Manual;
        sendOk(rid);
    }
    void cmdHeaterOff(const String& rid, JsonDocument& doc) {
    ...
    // ── Pump ────────────────────────────────────────────
    void cmdPumpOn(const String& rid, JsonDocument& doc) {
        EventBus::instance().publish(EventType::PumpStateChanged, true);
        sendOk(rid);
    }
    void cmdPumpOff(const String& rid, JsonDocument& doc) {
        EventBus::instance().publish(EventType::PumpStateChanged, false);
        sendOk(rid);
    }
    // ── PID Settings ────────────────────────────────────
    void cmdSetPid(const String& rid, JsonDocument& doc) {
    ...
    // ── Recipe: List ────────────────────────────────────
    void cmdRecipeList(const String& rid, JsonDocument& doc) {
    ...
    // ── Recipe: Load ────────────────────────────────────
    void cmdRecipeLoad(const String& rid, JsonDocument& doc) {
    ...
    // ── Recipe: Save ────────────────────────────────────
    void cmdRecipeSave(const String& rid, JsonDocument& doc) {
        if (!_sd) { sendError(rid, "No SD card"); return; }
        String filename = doc["file"] | "";
    ...
    // ── Recipe: Delete ──────────────────────────────────
    void cmdRecipeDelete(const String& rid, JsonDocument& doc) {
        if (!_sd) { sendError(rid, "No SD card"); return; }
        String filename = doc["file"] | "";
        if (!SDCardPlugin::isValidRecipeFilename(filename)) {
            sendError(rid, "Invalid filename"); return;
    ...
    // ── Recipe: Start ───────────────────────────────────
    void cmdRecipeStart(const String& rid, JsonDocument& doc) {
    ...
    // ── Recipe: Stop ────────────────────────────────────
    void cmdRecipeStop(const String& rid, JsonDocument& doc) {
        if (_recipe) _recipe->stop();
        sendOk(rid);
    }
    // ── Recipe: Pause ───────────────────────────────────
    void cmdRecipePause(const String& rid, JsonDocument& doc) {
        if (_recipe) _recipe->pause();
        sendOk(rid);
    }
    // ── Recipe: Resume ──────────────────────────────────
    void cmdRecipeResume(const String& rid, JsonDocument& doc) {
        if (_recipe) _recipe->resume();
        sendOk(rid);
    }
    // ── Recipe: Confirm ─────────────────────────────────
    void cmdRecipeConfirm(const String& rid, JsonDocument& doc) {
    ...
    void cmdSettingsSet(const String& rid, JsonDocument& doc) {
        float kp = doc["kp"] | gState.pidKp;
        float ki = doc["ki"] | gState.pidKi;
        float kd = doc["kd"] | gState.pidKd;
    ...
    void cmdStatus(const String& rid, JsonDocument& doc) {
        // Telemetry will be sent on next cycle; send one now
        sendOk(rid);
    }
    // ── Recovery: Resume ────────────────────────────────
    void cmdRecoveryResume(const String& rid, JsonDocument& doc) {
    ...
    // ── Recovery: Discard ───────────────────────────────
    void cmdRecoveryDiscard(const String& rid, JsonDocument& doc) {
        RecoveryManager::instance().clearRecovery();
        gState.hasRecoveryData = false;
    ...
    // networks).
    void cmdWifiConfig(const String& rid, JsonDocument& doc) {
    ...
    // ── WiFi: Connect ───────────────────────────────────
    void cmdWifiConnect(const String& rid, JsonDocument& doc) {
        if (!_wifi) { sendError(rid, "WiFi not available"); return; }
        _wifi->connect();
        sendOk(rid);
    }
    // ── WiFi: Disconnect ────────────────────────────────
    void cmdWifiDisconnect(const String& rid, JsonDocument& doc) {
        if (!_wifi) { sendError(rid, "WiFi not available"); return; }
        _wifi->disconnect();
        sendOk(rid);
    }
    // ── WiFi: Status ────────────────────────────────────
    void cmdWifiStatus(const String& rid, JsonDocument& doc) {
        if (!_wifi) { sendError(rid, "WiFi not available"); return; }
        _wifi->sendWiFiStatus();
        sendOk(rid);
    }
    // ── OTA: Check ──────────────────────────────────────
    void cmdOtaCheck(const String& rid, JsonDocument& doc) {
        if (!_ota) { sendError(rid, "OTA not available"); return; }
        _ota->checkForUpdate();
        sendOk(rid);
    }
    // ── OTA: Install ────────────────────────────────────
    void cmdOtaInstall(const String& rid, JsonDocument& doc) {
        if (!_ota) { sendError(rid, "OTA not available"); return; }
        _ota->installUpdate();
    ...
    // ── Ramp: Stop ──────────────────────────────────────
    void cmdRampStop(const String& rid, JsonDocument& doc) {
        if (!_ramp) { sendError(rid, "Ramp not available"); return; }
        _ramp->stopRamp();
        sendOk(rid);
    }
    // ── Brew Log: Start ─────────────────────────────────
    void cmdLogStart(const String& rid, JsonDocument& doc) {
        if (!_brewLog) { sendError(rid, "BrewLog not available"); return; }
        _brewLog->start();
        sendOk(rid);
    }
    // ── Brew Log: Stop ──────────────────────────────────
    void cmdLogStop(const String& rid, JsonDocument& doc) {
        if (!_brewLog) { sendError(rid, "BrewLog not available"); return; }
        _brewLog->stop();
        sendOk(rid);
    }
    // ── Brew Log: Export ────────────────────────────────
    void cmdLogExport(const String& rid, JsonDocument& doc) {
        if (!_brewLog) { sendError(rid, "BrewLog not available"); return; }
    ...
    // ── Boil Timer: Start ───────────────────────────────
    void cmdBoilStart(const String& rid, JsonDocument& doc) {
    ...
    // ── Boil Timer: Stop ────────────────────────────────
    void cmdBoilStop(const String& rid, JsonDocument& doc) {
        if (!_boilTimer) { sendError(rid, "BoilTimer not available"); return; }
        _boilTimer->stop();
        sendOk(rid);
    }
    // ── Boil Timer: Pause ───────────────────────────────
    void cmdBoilPause(const String& rid, JsonDocument& doc) {
        if (!_boilTimer) { sendError(rid, "BoilTimer not available"); return; }
        _boilTimer->pause();
        sendOk(rid);
    }
    // ── Boil Timer: Resume ──────────────────────────────
    void cmdBoilResume(const String& rid, JsonDocument& doc) {
        if (!_boilTimer) { sendError(rid, "BoilTimer not available"); return; }
        _boilTimer->resume();
        sendOk(rid);
    }
    // ── Boil Timer: Add Addition ────────────────────────
    void cmdBoilAdd(const String& rid, JsonDocument& doc) {
        if (!_boilTimer) { sendError(rid, "BoilTimer not available"); return; }
    ...
    // ── Mash-Out: Set ───────────────────────────────────
    void cmdMashoutSet(const String& rid, JsonDocument& doc) {
    ...
    // ── RTC: Set ────────────────────────────────────────
    void cmdRtcSet(const String& rid, JsonDocument& doc) {
    ...
    // ── RTC: Sync NTP ───────────────────────────────────
    void cmdRtcSync(const String& rid, JsonDocument& doc) {
        if (!_rtc) { sendError(rid, "RTC not available"); return; }
    ...
    void cmdRtcTzSet(const String& rid, JsonDocument& doc) {
        if (!_rtc) { sendError(rid, "RTC not available"); return; }
    ...
    // ── Timer: Start ────────────────────────────────────
    void cmdTimerStart(const String& rid, JsonDocument& doc) {
    ...
    // ── Timer: Stop ─────────────────────────────────────
    void cmdTimerStop(const String& rid, JsonDocument& doc) {
        if (!_timer) { sendError(rid, "Timer not available"); return; }
        _timer->stop();
        sendOk(rid);
    }
    // ── Timer: Pause ────────────────────────────────────
    void cmdTimerPause(const String& rid, JsonDocument& doc) {
        if (!_timer) { sendError(rid, "Timer not available"); return; }
        _timer->pause();
        sendOk(rid);
    }
    // ── Timer: Resume ───────────────────────────────────
    void cmdTimerResume(const String& rid, JsonDocument& doc) {
        if (!_timer) { sendError(rid, "Timer not available"); return; }
        _timer->resume();
        sendOk(rid);
    }
    // ── Timer: Add Time ─────────────────────────────────
    void cmdTimerAdd(const String& rid, JsonDocument& doc) {
        if (!_timer) { sendError(rid, "Timer not available"); return; }
    ...
    // ── Auto-Tune: Start ────────────────────────────────
    void cmdAutotuneStart(const String& rid, JsonDocument& doc) {
    ...
    // ── Auto-Tune: Stop ─────────────────────────────────
    void cmdAutotuneStop(const String& rid, JsonDocument& doc) {
        if (!_autoTune) { sendError(rid, "AutoTune not available"); return; }
        _autoTune->stop();
        sendOk(rid);
    }
    // ── Timer: Alarm (absolute time) ────────────────────
    void cmdTimerAlarm(const String& rid, JsonDocument& doc) {
        if (!_timer) { sendError(rid, "Timer not available"); return; }
    ...
    // ── Scheduler: Set ──────────────────────────────────
    void cmdSchedulerSet(const String& rid, JsonDocument& doc) {
    ...
    // ── Scheduler: Stop ─────────────────────────────────
    void cmdSchedulerStop(const String& rid, JsonDocument& doc) {
        if (!_scheduler) { sendError(rid, "Scheduler not available"); return; }
        _scheduler->cancel();
        sendOk(rid);
    }
    ...
    void cmdSettingsThermalSet(const String& rid, JsonDocument& doc) {
    ...
    void cmdLidStateSet(const String& rid, JsonDocument& doc) {
    ...
    // ── LossTune ────────────────────────────────────────
    void cmdLosstuneStart(const String& rid, JsonDocument& doc) {
    ...
    void cmdLosstuneCancel(const String& rid, JsonDocument& doc) {
        if (!_lossTune) { sendError(rid, "losstune_unavailable"); return; }
        _lossTune->cancel();
        sendOk(rid);
    }
    void cmdLosstuneAccept(const String& rid, JsonDocument& doc) {
        if (!_lossTune) { sendError(rid, "losstune_unavailable"); return; }
        if (!_lossTune->accept()) { sendError(rid, "not_in_result"); return; }
        sendOk(rid);
    }
    void cmdLosstuneReject(const String& rid, JsonDocument& doc) {
        if (!_lossTune) { sendError(rid, "losstune_unavailable"); return; }
    ...
    void cmdSettingsCalSet(const String& rid, JsonDocument& doc) {
    ...
    void cmdWatchdogReset(const String& rid, JsonDocument& doc) {
    ...
    void cmdWatchdogConfig(const String& rid, JsonDocument& doc) {
        if (!_watchdog) { sendError(rid, "wd_unavailable"); return; }
    ...
    void cmdHeaterConfig(const String& rid, JsonDocument& doc) {
    ...
    void cmdDeviceRename(const String& rid, JsonDocument& doc) {
    ...
    void cmdFactoryReset(const String& rid, JsonDocument& doc) {
