...
public:
    ...
    // buffer OR a directory iterator. The previous test of `_data` alone
    // made `SD.open("/recipes")` look closed to SDCardPlugin::listRecipes,
    // which then short-circuited and returned an empty list in the sim.
    ...
    void setDirectoryEntries(std::vector<std::pair<std::string, std::vector<uint8_t>*>> e) {
        // _valid must flip with the directory marker — listRecipes()
        // uses `if (!dir || !dir.isDirectory())` and the previous
        // default-constructed File left _valid=false, so the directory
        // case always short-circuited even though entries were present.
